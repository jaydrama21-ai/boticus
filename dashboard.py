...
with tab8:
    ...
    changelog = [
        ...
        {
            "version": "v0.8 — Schedule + Reliability",
            "date":    "Jun 2026",
            "kind":    "win",
            "items": [
                "Aggressive cron schedule — 64 entries saturate open & close windows minute-by-minute",
                "Open window (9:29–9:45 AM ET): per-minute triggers, worst case 2-min coverage",
                "Close window (3:30–4:05 PM ET): per-minute triggers, guaranteed EOD coverage",
                "Mid-day: every-5-min so even at 50% skip rate coverage stays at 10-min",
                "Keep-alive cron at midnight UTC prevents GitHub disabling the schedule",
                "permissions: contents: write added so bot can commit state back to repo",
                "git commit step after every run — Streamlit Cloud now gets live data automatically",
                "Empty-commit guard: only pushes if bot_state files actually changed",
            ]
        },
    ]
